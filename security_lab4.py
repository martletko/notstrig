...
def hash_value(value: bytes) -> str:
    '''
    пользователь ввел пароль, генерируем ему хэш значение по паролю, возвращает хеш значение
    '''
...
if __name__ == "__main__":
    ...
    passw = bytes('123', 'utf-8') # сгенерировали ключ на основе пользовательского пароля

    sk1 = gen_user_secret_key(passw) # сгенерировали ключ на основе пользовательского пароля
    dc = encrypt(sk,sk1) # зашифровали рандомные байты  помощью пользовательского пароля

    dc = dc.hex()
    dc = bytes.fromhex(dc)

    dc = decrypt(dc, sk1) # расшифровали рандомные байты с помощью ключа на основе пользовательского праоля
    ...
    теперь его надо зашифровать с помощью мастер ключа ген юзер секрет кей возвращает мастер ключ #  
    теперь необходимо зашифровать мастер ключ с помощью шифрования енкрипт и декрипт  #
    '''
